...
def glyph_O(w):
    """Halka + dörtlü pencere kayıtları (kayıtlar ana gövdeden ince)."""
    inner = R - w / 2
...
KERN = {('G','L'): -3, ('L','O'): -15, ('O','B'): -7, ('B','A'): 7, ('A','L'): -7}


def wordmark(w=13.0, track=28.0, thin_ratio=0.62, ink='#1A1A18', cati='sacak'):
    """GLOBAL kelimesini tek <g> olarak döndürür; (svg_parcasi, genislik)."""
    seq = [('G', glyph_G()), ('L', glyph_L()), ('O', glyph_O(w)),
...
def lockup(w=13.0, track=28.0, ink='#1A1A18', desc_ink=None, stacked=False, cati='sacak',
           desc='YAPI GELİŞTİRME A.Ş.'):
    """Wordmark + açıklama satırı. Tam SVG belgesi döndürür."""
    desc_ink = desc_ink or ink
    mark, mw = wordmark(w, track, ink=ink, cati=cati)
    pad = w / 2 + 2
    ...
    if stacked:
        ...
        body = (f'<g transform="translate({p(pad)},{p(pad)})">{mark}'
                f'<text x="{p(mw/2)}" y="{p(CAP + gap)}" text-anchor="middle" '
...
def amblem(kind='pencere', w=13.0, ink='#1A1A18', cati='sacak'):
    """Tek başına amblem: 'pencere' (O) veya 'cati' (A)."""
    pad = w / 2 + 4
    if kind == 'pencere':
        gl = glyph_O(w)
    else:
...
if __name__ == '__main__':
    files = []
    for name, cfg in KONSEPT.items():
        for tag, ink in (('siyah', INK), ('beyaz', '#FFFFFF')):
            fn = f'{name}-{tag}.svg'
            open(os.path.join(OUT, fn), 'w', encoding='utf-8').write(lockup(ink=ink, **cfg))
            files.append(fn)
    for kind in ('pencere', 'cati'):
        fn = f'amblem-{kind}.svg'
        open(os.path.join(OUT, fn), 'w', encoding='utf-8').write(amblem(kind))
        files.append(fn)
